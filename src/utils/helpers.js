...
const createTourUpdate = async (customTourId, updateType, updateSummary, updatedBy) => {
  ...
  try {
    // Convert old format to new schema requirements
    const updateTitle = updateSummary || `${updateType} update`;
    const updateContent = updateSummary || `A ${updateType} update has been made to the tour.`;
    ...
    const tourUpdate = new TourUpdate({
      ...
      is_published: true // Auto-publish backend-generated updates
    });
