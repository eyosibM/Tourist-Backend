...
class ImageUploadService {
  ...
  static createUploadMiddleware(folder = 'images', allowedTypes = ['image/jpeg', 'image/png'], maxSize = 5 * 1024 * 1024) {
    ...
    // Custom S3 storage configuration (AWS SDK v3 compatible)
    const upload = multer({
    ...
    // Add custom S3 upload middleware
          ...
          if (req.files && req.files.length > 0) {
            try {
              for (let i = 0; i < req.files.length; i++) {
                ...
                // Add S3 location to file object
                file.location = `https://${process.env.S3_BUCKET_NAME}.s3.${process.env.AWS_REGION || 'us-east-1'}.amazonaws.com/${fileName}`;
                file.key = fileName;
                file.bucket = process.env.S3_BUCKET_NAME;
              }
            } catch (uploadError) {
              console.error('S3 upload error:', uploadError);
              return next(new Error('Failed to upload files to S3'));
            }
          }
