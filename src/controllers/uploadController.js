...
const uploadTourImage = async (req, res) => {
  try {
    ...
    res.json({
      message: `Tour ${image_type || 'media'} uploaded successfully`,
      file_url: fileUrl, // Frontend expects file_url
      fileUrl, // Keep for backward compatibility
      fileName,
      fileSize,
...
const uploadMultipleTourImages = async (req, res) => {
  try {
    ...
    const uploadedFiles = req.files.map(file => ({
      ...
      file_url: file.location || file.path, // Frontend expects file_url
      fileUrl: file.location || file.path, // Keep for backward compatibility
...
// Upload tour media (images or videos to S3)
const uploadTourMedia = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'No file provided' });
    }
    ...
    // Upload media to S3 using MediaUploadService
    ...
    console.error('Tour media upload error:', error);
    
    res.status(500).json({ 
      error: 'Failed to upload tour media to S3',
