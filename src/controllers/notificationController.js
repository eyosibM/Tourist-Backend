...
const getUserNotifications = async (req, res) => {
  try {
    ...
    if (related_entity_type) {
      query['related_entities.custom_tour_id'] = { $exists: true };
    }

    const notifications = await Notification.find(query)
      .populate('sender_id', 'first_name last_name email profile_picture')
      .populate('related_entities.custom_tour_id', 'tour_name')
      .sort({ created_date: -1 })
      .skip(skip)
      .limit(limitNum);
    ...
    const transformedNotifications = notifications.map(notification => ({
      ...
      read: notification.is_read, // Alias for compatibility
      related_entity_type: notification.related_entities.custom_tour_id ? 'custom_tour' : null,
      related_entity_id: notification.related_entities.custom_tour_id,
      created_date: notification.created_date,
      updated_date: notification.updated_date,
      ...
      sender: notification.sender_id ? {
        ...
        first_name: notification.sender_id.first_name,
        last_name: notification.sender_id.last_name,
        email: notification.sender_id.email,
        profile_picture: notification.sender_id.profile_picture
      } : null,
      related_tour_name: notification.related_entities.custom_tour_id?.tour_name
    }));

    if (page && limit) {
      res.json(buildPaginationResponse(transformedNotifications, total, page, limit));
    } else {
...
const getNotificationCount = async (req, res) => {
  try {
    ...
    const [total, unread] = await Promise.all([
      Notification.countDocuments({ recipient_id: userId }),
...
const getNotificationById = async (req, res) => {
  try {
    ...
    // Transform notification for frontend compatibility
  ...
  try {
    ...
    if (channels.push) {
      try {
        ...
        console.error('Error queuing push notification:', pushError);
