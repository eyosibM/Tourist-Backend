...
const getCalendarEntries = async (req, res) => {
  try {
    ...
    // Check access permissions for tour templates
    if (tour_template_id) {
      ...
      // Tour templates are system-wide and accessible to all provider admins and system admins
      if (req.user.user_type !== 'provider_admin' && req.user.user_type !== 'system_admin') {
        return res.status(403).json({ error: 'Access denied' });
      }
    }
...
const createCalendarEntry = async (req, res) => {
  try {
    console.log('Received calendar entry data:', JSON.stringify(req.body, null, 2));
    ...
    // Check access permissions for tour templates
    if (entryData.tour_template_id) {
      const template = await TourTemplate.findById(entryData.tour_template_id);
      if (!template) {
        return res.status(400).json({ error: 'Tour template not found' });
      }
...
const updateCalendarEntry = async (req, res) => {
  try {
    ...
    // Check access permissions for tour templates
    if (currentEntry.tour_template_id) {
      const template = await TourTemplate.findById(currentEntry.tour_template_id);
      if (template) {
        // Tour templates are system-wide and accessible to all provider admins and system admins
        if (req.user.user_type !== 'provider_admin' && req.user.user_type !== 'system_admin') {
          return res.status(403).json({ error: 'Access denied' });
...
const deleteCalendarEntry = async (req, res) => {
  try {
    ...
    // Check access permissions for tour templates
    if (entry.tour_template_id) {
      const template = await TourTemplate.findById(entry.tour_template_id);
      if (template) {
        // Tour templates are system-wide and accessible to all provider admins and system admins
        if (req.user.user_type !== 'provider_admin' && req.user.user_type !== 'system_admin') {
          return res.status(403).json({ error: 'Access denied' });
