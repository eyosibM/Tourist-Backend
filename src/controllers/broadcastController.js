...
const createBroadcast = async (req, res) => {
    try {
        ...
        // Provider admins can only access tours from their provider
        if (req.user.user_type === 'provider_admin') {
            tourQuery.provider_id = req.user.provider_id;
        }
        // System admins can access all tours (no additional restrictions)
        ...
        if (status === 'published') {
            console.log('📢 Sending broadcast notifications for published broadcast');
            await NotificationService.sendBroadcastNotification(populatedBroadcast);
        }

        console.log('✅ Broadcast created successfully:', populatedBroadcast._id);
