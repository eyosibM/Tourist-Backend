...
# =============================================================================
# HTTPS SETUP SCRIPT FOR API.TOURLICITY.COM
# =============================================================================
# This script sets up HTTPS with Let's Encrypt for api.tourlicity.com
# =============================================================================

set -e  # Exit on any error

echo "🔒 Setting up HTTPS for api.tourlicity.com..."
echo "=============================================="
...
# Create nginx configuration with SSL for api.tourlicity.com
cat > docker/nginx/nginx-api-ssl.conf << 'EOF'
events {
...
http {
    ...
    server {
        listen 80;
        server_name api.tourlicity.com;
    ...
    server {
        ...
        add_header Access-Control-Allow-Origin "https://tourist-frontend-c8ji.vercel.app" always;
...
echo -e "${YELLOW}🐳 Step 6: Creating Docker Compose for HTTPS API...${NC}"

# Create HTTPS version of docker-compose for API domain
cat > docker-compose.api-https.yml << 'EOF'
version: '3.8'
...
services:
  ...
  # Tourlicity API - Internal port only
  api:
    build: .
    ...
    environment:
      ...
      # Core Configuration - HTTPS API URL
      NODE_ENV: production
      PORT: 5000
      BASE_URL: https://api.tourlicity.com
  ...
  # Nginx Reverse Proxy with SSL for API domain
  nginx:
    image: nginx:alpine
    ...
    ports:
      ...
      - "443:443"
    volumes:
      - ./docker/nginx/nginx-api-ssl.conf:/etc/nginx/nginx.conf
...
docker-compose -f docker-compose.api-https.yml up -d --build

# Step 8: Set up certificate auto-renewal
echo -e "${YELLOW}🔄 Step 8: Setting up certificate auto-renewal...${NC}"

# Create renewal script for API domain
cat > renew-api-ssl.sh << 'EOF'
#!/bin/bash
# SSL Certificate Renewal Script for api.tourlicity.com

echo "🔄 Renewing SSL certificates for api.tourlicity.com..."
...
sudo cp /etc/letsencrypt/live/api.tourlicity.com/fullchain.pem docker/nginx/ssl/
sudo cp /etc/letsencrypt/live/api.tourlicity.com/privkey.pem docker/nginx/ssl/
sudo chmod 644 docker/nginx/ssl/fullchain.pem
sudo chmod 600 docker/nginx/ssl/privkey.pem
sudo chown $USER:$USER docker/nginx/ssl/*
...
chmod +x renew-api-ssl.sh

# Add to crontab for automatic renewal (runs twice daily)
(crontab -l 2>/dev/null; echo "0 */12 * * * $(pwd)/renew-api-ssl.sh >> $(pwd)/api-ssl-renewal.log 2>&1") | crontab -
...
echo ""
echo -e "${GREEN}🎉 HTTPS SETUP COMPLETE FOR API.TOURLICITY.COM!${NC}"
echo "=================================================="
echo -e "${BLUE}📍 Your API is now available at:${NC}"
echo "   🔒 https://api.tourlicity.com"
echo "   📚 https://api.tourlicity.com/api-docs"
echo "   ❤️  https://api.tourlicity.com/health"
echo ""
echo -e "${BLUE}🔧 Testing HTTPS:${NC}"
curl -s https://api.tourlicity.com/health || echo "   ⏳ HTTPS may take a moment to be ready..."
echo ""
echo -e "${BLUE}🔄 Certificate Auto-Renewal:${NC}"
echo "   📅 Certificates will auto-renew twice daily"
echo "   📋 Check renewal logs: tail -f api-ssl-renewal.log"
echo "   🔧 Manual renewal: ./renew-api-ssl.sh"
echo ""
echo -e "${GREEN}✅ HTTPS setup successful for api.tourlicity.com!${NC}"